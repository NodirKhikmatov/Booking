...
import { shapeIntoMongoDbObjectId, lookupMember, lookUpAuthMemberLiked } from '../../libs/config';
...
export class BoardArticleService {
	...
	public async createBoardArticle(memberId: ObjectId, input: BoardArticleInput): Promise<BoardArticle> {
		...
		try {
			...
			await this.memberService.memberStateEditor({ _id: memberId, targetKey: 'memberArticles', modifier: 1 });
	...
	public async getBoardArticle(memberId: ObjectId, articleId: ObjectId): Promise<BoardArticle> {
		...
		targetBoardArticle.memberData = await this.memberService.getMember(targetBoardArticle.memberId);
	...
	public async getBoardArticles(memberId: ObjectId, input: BoardArticlesInquiry): Promise<BoardArticles> {
		...
		if (input.search?.memberId) {
			match.memberId = shapeIntoMongoDbObjectId(input.search.memberId);
		}
